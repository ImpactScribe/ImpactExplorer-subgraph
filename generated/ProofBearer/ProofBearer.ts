...
export class ProofBearer extends ethereum.SmartContract {
  static bind(address: Address): ProofBearer {
    return new ProofBearer("ProofBearer", address);
  }
  ...
  TOGGLE_ROLE(): Bytes {
    let result = super.call("TOGGLE_ROLE", "TOGGLE_ROLE():(bytes32)", []);

    return result[0].toBytes();
  }

  try_TOGGLE_ROLE(): ethereum.CallResult<Bytes> {
    let result = super.tryCall("TOGGLE_ROLE", "TOGGLE_ROLE():(bytes32)", []);
    if (result.reverted) {
      return new ethereum.CallResult();
    }
    let value = result.value;
  ...
  tokenAccount(tokenId: BigInt): Address {
    let result = super.call("tokenAccount", "tokenAccount(uint256):(address)", [
      ethereum.Value.fromUnsignedBigInt(tokenId),
    ]);
  ...
  try_tokenAccount(tokenId: BigInt): ethereum.CallResult<Address> {
    let result = super.tryCall(
      "tokenAccount",
  ...
  transferAllowed(): boolean {
    let result = super.call("transferAllowed", "transferAllowed():(bool)", []);

    return result[0].toBoolean();
  }

  try_transferAllowed(): ethereum.CallResult<boolean> {
    let result = super.tryCall(
      "transferAllowed",
...
export class ToggleTransfersCall extends ethereum.Call {
  get inputs(): ToggleTransfersCall__Inputs {
    return new ToggleTransfersCall__Inputs(this);
  }

  get outputs(): ToggleTransfersCall__Outputs {
    return new ToggleTransfersCall__Outputs(this);
  }
}

export class ToggleTransfersCall__Inputs {
  _call: ToggleTransfersCall;

  constructor(call: ToggleTransfersCall) {
    this._call = call;
  }
...
export class ToggleTransfersCall__Outputs {
  _call: ToggleTransfersCall;
